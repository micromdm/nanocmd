...
func TestMongoDBStorage(t *testing.T) {
	...
	if testDatabase == "" {
		testDatabase = "nanocmd_cmdplan_storage_test"
	}

	testCollection := "subsystem_cmdplans_" + strings.NewReplacer("/", "_").Replace(t.Name())
	s, err := New(
		WithURI(testURI),
	...
	test.TestCMDPlanStorage(t, func() storage.Storage { return s })
}
